Cohorts = (function() {
    ...
    var GoogleAnalyticsAdapter = {
        nameSpace: 'cohorts',
        assignments: {},
        customVar: [1, 'TestGroups'],
        onInitialize: function(inTest, testName, cohort) {
            if (inTest)
                Utils.log('GA bucketing: in test ' + testName + ', assigned to ' + cohort);
            else
                Utils.log('GA bucketing: not in test ' + testName);

            if(!inTest) return;

            if(Utils.size(this.assignments) == 0) {
                if(!window['_gaq'])
                ...
                _gaq.push(function() {
                    var parts = [];
                    for (var test in self.assignments)
                        parts.push(test + '=' + self.assignments[test]);
                    _gaq.push(['_setCustomVar', self.customVar[0], self.customVar[1], parts.join('&')]);
                });
            }

            this.assignments[testName] = cohort;
        }
    };
    ...
    var Test = (function() {        
        ...
        var constructor = function(options) {
            ...
            if(!this.options.storageAdapter) {
                if (!window.Scribd || !Scribd.CohortsAdapter)
                    throw("Couldn't find Scribd.CohortsAdapter");
                this.options.storageAdapter = Scribd.CohortsAdapter;
            }
        ...
        constructor.prototype = {
            run: function() {
                ...
                if(in_test) {
                    ...
                    if(this.options.onNotInTest)
                        this.options.onNotInTest();
                }
            },
            inTest: function() {
            ...
            setCookie: function(name, value) {
                ...
                data[this.options.name] = data[this.options.name] || {};

                data[this.options.name][name] = value;
                Cookies.set(cookieName, data);
            },
            getCookie: function(name) {
                var data = Cookies.get(cookieName) || {};
                if (!data[this.options.name]) return null;

                return data[this.options.name][name];
            }
        };
